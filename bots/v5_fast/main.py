...
class BfsBureau:
    ...
    def find_bridge_route(cls, start: Position, core_pos_list: list[tuple[int, int]]):
        ...
        # First hop: knight moves + diagonal + straight jumps
        ni = si + 168
        if dist[ni] == 1000000:
            dist[ni] = 1
            _fh = (start.x + 3, start.y + 0)
            first_hop[ni] = _fh
            if ni in core_set:
                return 1, _fh
            _qa(ni)
