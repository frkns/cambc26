...
# ---===
    ...
    def fcompute(cls):
        ...
        # within the last 4 rounds.  Propagated leaf→root so
        ...
        for u in active:
            if (
                _sight_last_id[u] != -1
                and (_cur_round - _sight_last_round[u]) <= 4
            ):
        ...
        for u in reversed(q):
            p = ns[u].up
            if p is None:                        # u is a root / sink
                pressure[u] = flow[u]
                k = kind[u]
                # foundry roots own themselves
                fo = u if (k == _ALLY_CONSUMER_K and u in _foundry_set) else -1
            elif kind[p]:                        # parent is a sink boundary
                pressure[u] = flow[u]
                k = kind[p]
                # direct children of a foundry belong to it
                fo = p if (kind[p] == _ALLY_CONSUMER_K and p in _foundry_set) else -1
            else:
