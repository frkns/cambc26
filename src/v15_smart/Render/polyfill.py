# need to polyfill with TYPE_CHECKING=True after Controller wrap
# careful with GameConstants!
# (OK, just don't use this! use polyfill_controller instead.)
...
class GameConstants:
    ...
    SENTINEL_AMMO_COST = 5
    SENTINEL_STUN_DURATION = 2
...
if TYPE_CHECKING:
    class Controller:  # type: ignore[no-redef]
        ...
        def heal(self, position: Position) -> None:
            """Heal all friendly entities on this builder bot's own tile by 4 HP. If both a builder bot and
            a friendly building are on the tile, both are healed. Costs 1 titanium and one action cooldown.
        ...
        def can_heal(self, position: Position) -> bool:
            ...
            position must equal the builder bot's current position.
        ...
        def can_fire(self, target: Position) -> bool:
            ...
            Gunners may target any occupied tile in their firing line; markers do not block farther targets.
        ...
        def fire(self, target: Position) -> None:
            ...
            Gunners may fire through markers at occupied tiles behind them.
            Use launch() instead for launchers.
            Raises GameError if not legal."""
            ...

        def get_gunner_target(self) -> Position | None:
            """Return the position of the closest occupied tile in the gunner's facing direction,
            or None if nothing is in range. Markers may be returned even though farther tiles behind them
            are also targetable. Only valid on gunners."""
            ...
