...
class HealTargetInfo:
    __slots__ = (
        ...
        'has_enemy_bot', 'bfs_dist_adj', 'entity_type', 'core_info', 'is_launcher'
    )
    ...
    def is_better_than(a: HealTargetInfo, b: HealTargetInfo) -> bool:
        ...
        if a.is_launcher: return False
        if b.is_launcher: return True
...
class HealTargeter:
    ...
    def get_best_target_info(cls) -> HealTargetInfo | None:
        target = cls.wrapped_get_best_target_info()
        if target is not None:
            return target

        my_pos = Globals.my_pos

        if cls.core_info is not None and cls.core_info.building_hp <= 450:
            return cls.core_info

        return None

    @classmethod
    def wrapped_get_best_target_info(cls) -> HealTargetInfo | None:
        targets = cls.targets
        if not targets:
            return None
    ...
    def fill(cls):
        ...
        for pos, x, y, idx, ti in Map.proc_nearby_tiles:
            ...
            info.is_launcher = ti.entity_type == EntityType.LAUNCHER
            info.is_turret = ti.has_turret
            ...
            if ti.entity_type == EntityType.CORE:
                cls.core_info = info
